...
def _set_gn_for_layer(layer: nn.Sequential, C: int, num_groups: int = 32):
    """
    将一层 Sequential 里的所有 BasicBlock 的 BN 替换为 GN。
    若该层首个 block 带下采样，则将 shortcut[1] 的 BN 也替换为 GN。
    """
    for i, block in enumerate(layer):
        # 主分支两处归一化
        block.bn1 = nn.GroupNorm(num_groups=num_groups, num_channels=C)
        block.bn2 = nn.GroupNorm(num_groups=num_groups, num_channels=C)
        # 下采样分支（只有第一个 block 可能有）
        if i == 0 and hasattr(block, "shortcut") and isinstance(block.shortcut, nn.Sequential):
            if len(block.shortcut) >= 2 and isinstance(block.shortcut[1], nn.BatchNorm2d):
                block.shortcut[1] = nn.GroupNorm(num_groups=num_groups, num_channels=C)

def tiny_resnet34(pretrained: bool = False, class_num: int = 200, progress: bool = True):
    # 34 层配置： [3, 4, 6, 3]
    res34 = tiny_ResNet(BasicBlock, [3, 4, 6, 3], class_num=class_num)

    # stem 的 BN → GN
    res34.bn1 = nn.GroupNorm(num_groups=32, num_channels=64)

    # 各 stage 的 BN → GN （通道分别为 64/128/256/512）
    _set_gn_for_layer(res34.layer1, C=64,  num_groups=32)
    _set_gn_for_layer(res34.layer2, C=128, num_groups=32)
    _set_gn_for_layer(res34.layer3, C=256, num_groups=32)
    _set_gn_for_layer(res34.layer4, C=512, num_groups=32)

    # 与你的 res18 一样做一个一致性检查（参数+缓冲一致）
    assert len(dict(res34.named_parameters()).keys()) == len(res34.state_dict().keys()), \
