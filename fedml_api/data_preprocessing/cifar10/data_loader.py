...
def partition_data( datadir, partition, n_nets, alpha, logger):
    ...
    if partition == 'n_cls':
        ...
        for i in range(n_client):
            cls_priors[i][random.sample(range(n_cls), int(alpha))] = 1.0 / 2 
...
def load_partition_data_cifar10( data_dir, partition_method, partition_alpha, client_number, batch_size, logger):
    ...
    transform_train, transform_test = _data_transforms_cifar10()
    cache_train_data_set=CIFAR10(data_dir, train=True, transform=transform_train, download=False)
    cache_test_data_set = CIFAR10(data_dir, train=False, transform=transform_test, download=False)
