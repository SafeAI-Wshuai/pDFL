...
def add_args(parser):
    ...
    parser.add_argument('--partition_alpha', type=float, default=5, metavar='PA',
    ...
    parser.add_argument('--head_epochs', type=int, default=2, metavar='EP',
                        help='local training epochs for each client')
    parser.add_argument('--body_epochs', type=int, default=2, metavar='EP',
                        help='local training epochs for each client')
...
if __name__ == "__main__":
    ...
    args.identity = "dfedpgp-"  + "part-"+data_partition
    ...
    for exper_index in range(args.num_experiments):
        ...
        dfedPgpAPI = DFedPGPAPI(dataset, device, args, model_trainer, logger, log_path=log_path)
        dfedPgpAPI.train()
